...
@Component({
  ...
  template: `
    <div class="flex flex-col h-full overflow-hidden">
      ...
      <div class="flex items-center justify-between mb-8 shrink-0 px-2">
        <div>
          ...
          <p class="text-xs font-mono text-zinc-500 mt-1">Orquestração técnica e execução de sprints Konig.</p>
        </div>
          ...
          >
            ...
            <div class="flex-1 p-4 space-y-4 overflow-y-auto custom-scrollbar min-h-[150px]">
              @for (task of getTasksByStatus(col); track task.id) {
                <div 
                  class="bg-zinc-950 border border-zinc-800 rounded-2xl p-5 hover:border-indigo-500/50 hover:shadow-2xl cursor-pointer transition-all group relative overflow-hidden"
                  draggable="true"
                ...
                >
                  ...
                  <div class="flex justify-between items-start mb-4">
                    ...
                    <span class="px-2 py-0.5 rounded text-[9px] font-black border uppercase tracking-tighter"
                      [class.bg-rose-500.text-white]="task.priority === 'Urgente'"
    ...
    <!-- New Task Modal (Jira Style) -->
    <app-nexus-modal
      [isOpen]="isModalOpen()"
      title="Novo Ticket de Engenharia"
      subtitle="Definição técnica e planejamento de sprint"
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div class="space-y-6">
          ...
          <div class="space-y-2">
            ...
            <textarea [(ngModel)]="newTask.description" rows="5" placeholder="Critérios de aceite e contexto técnico..." class="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-white focus:border-indigo-500 outline-none resize-none text-sm"></textarea>
          </div>
        ...
        <div class="space-y-6">
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
              <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Tipo</label>
              <select [(ngModel)]="newTask.type" class="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-white focus:border-indigo-500 outline-none">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="space-y-2">
              <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Categoria</label>
              <input [(ngModel)]="newTask.category" placeholder="Backend, UI..." class="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-white focus:border-indigo-500 outline-none">
            </div>
            <div class="space-y-2">
              <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Stack</label>
              <input [(ngModel)]="newTask.stack" placeholder="Angular, Node..." class="w-full bg-zinc-900 border border-zinc-800 rounded-xl p-4 text-white focus:border-indigo-500 outline-none">
            </div>
          </div>

          <div class="space-y-2">
            <label class="text-[10px] font-black text-zinc-500 uppercase tracking-widest">Story Points (Estimativa)</label>
